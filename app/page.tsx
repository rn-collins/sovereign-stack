...
const meetingSlides = [
  { label: "What changed", title: "Purple Maiʻa has now publicly named and demonstrated its Sovereign Stack.", body: "ʻĀina Foundry describes open models, local compute, AI coding agents, and an edge-hardware path toward community-controlled infrastructure. That public development sharpens the question behind this independent prototype.", proof: "Public record, not internal knowledge. RN has previously contacted Purple Maiʻa; no commission, endorsement, adoption, or organizational validation is implied." },
  ...
  { label: "Author’s role", title: "RN translates governance into working legal-technical infrastructure.", body: "RN combines AI product building, evidence architecture, governance and legal research, learning design, neuroscience, and human-systems analysis, and builds editable tools rather than static reports.", proof: "Any role would be implementation architect under authority designated by Purple Maiʻa and relevant authorities—never Indigenous authority or the source of Hawaiian values." },
...
export default function Home() {
  ...
  function returnToBeginning() { setView("review"); window.scrollTo({ top: 0, behavior: "smooth" }); }
  ...
  function exportMeetingBrief() {
    downloadText("authority-layer-executive-brief.md", `# Authority Layer\n\n**An independent governance prototype for Purple Maiʻa's Sovereign Stack**  \n**By Rayven-Nikkita (RN) Collins · Version 1.1 · 12 September 2026**  \n**RN has previously been in contact with Purple Maiʻa. This prototype was not commissioned, adopted, endorsed, or organizationally validated by Purple Maiʻa.**\n\n## Why this is being resurfaced now\n\nOn 2 September 2026, ʻĀina Foundry publicly presented its Sovereign Stack: open models, local compute, AI coding agents, and an edge-hardware path toward community-controlled infrastructure. That development sharpens—but does not prove—the question explored here.\n\n## The question\n\nHow could purpose, standing, knowledge boundaries, permissions, conditions, dissent, review, withdrawal, repair, migration, and retirement remain operational as the technical stack changes?\n\n## What is already built\n\nA working demonstration connects a pre-build decision gate, living authority record, review history, production questions, co-design process, and bounded pilot charter. Pause, refusal, withdrawal, and non-digitization are valid outcomes. This is a conversation object—not an adopted protocol or secure production system.\n\n## What RN is—and is not—claiming\n\nRN is proposing a testable mechanism, not claiming Purple Maiʻa lacks governance work. Purple Maiʻa may already address the need, define a different problem, or see no useful gap. RN's possible role is implementation architect working under authority designated by Purple Maiʻa and relevant authorities—not Indigenous authority or the source of Hawaiian values.\n\n## A possible first engagement\n\nA bounded, compensated discovery engagement: sponsor interview and preparation; one 90-minute governed co-design session; synthesis; and an editable decision brief. It would identify the real constraint, map standing and documentation boundaries, trace one non-sensitive scenario, and end with stop, revise, continue discovery, or invite a separately chartered pilot.\n\n## The pressure test\n\nDoes Purple Maiʻa's Sovereign Stack need an authority layer—or would this duplicate work already in place? This is a proposal for conversation, not a request to approve a framework, select KILO, disclose protected knowledge, authorize production, or consent to publication.\n\n## Boundaries\n\n- Purple Maiʻa and relevant community and cultural authorities define substance and standing.\n- Protected knowledge does not belong in this public browser demonstration.\n- Public excerpts require separate review and approval.\n- Interest, attendance, form completion, or download does not create authority or consent.\n- Revision, referral, redundancy, deferral, and stopping are successful findings.\n\nSources and their limits are documented in the site's Assumption ledger.\n`);
  }
  ...
  function exportRecord() {
    ...
    const link = document.createElement("a"); link.href = url; link.download = `${(projectName || "authority-layer-review").toLowerCase().replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "authority-layer-review"}.json`; link.click(); URL.revokeObjectURL(url);
  }
  ...
  function exportPilotCharter() {
    ...
    const payload = {
      document: "Authority Layer bounded pilot charter",
  ...
  return <main>
    ...
    <section className="hero">
      <div className="hero-copy">
        ...
        <p className="lede"><strong>Authority Layer</strong> asks how purpose, standing, knowledge boundaries, permissions, dissent, review, withdrawal, repair, migration, and retirement could remain operational across Purple Maiʻa’s publicly described Sovereign Stack.</p>
        <div className="hero-actions"><button className="primary light" onClick={() => document.getElementById("what-changed")?.scrollIntoView({behavior:"smooth",block:"start"})}>See what changed <span>↓</span></button><button className="text-link" onClick={() => selectView("review")}>Executive review</button><button className="text-link" onClick={() => selectView("gate")}>Try the working system</button></div>
      </div>
    ...
    <div className="scope-strip"><b>Independent prototype. Proposed basis for conversation—not a request for adoption.</b><span>RN has previously been in contact with Purple Maiʻa. Purple Maiʻa has not commissioned, adopted, endorsed, or organizationally validated this prototype. Any further work would begin by testing whether the hypothesized need is real, already addressed, or framed at the wrong layer.</span></div>

    <section id="what-changed" className="update-panel" aria-labelledby="update-title">
      <div><p className="overline">Why this is being resurfaced now</p><h2 id="update-title">Purple Maiʻa has now publicly named the technical Sovereign Stack.</h2><p>On 2 September 2026, ʻĀina Foundry presented a Sovereign Stack built around open models, local compute, AI coding agents, and an edge-hardware path toward community-controlled infrastructure. That development sharpens—but does not prove—the question this prototype tests.</p></div>
      <div className="precision-grid"><article><span>Public record</span><b>Purple Maiʻa is building sovereignty-first technical capacity.</b></article><article><span>RN hypothesis</span><b>Authority may need its own durable operational layer.</b></article><article><span>Working design</span><b>A gate, authority record, lifecycle controls, and bounded discovery path.</b></article><article><span>Still unknown</span><b>Useful, redundant, wrong layer—or worth pressure-testing?</b></article></div>
    </section>
    ...
    {view === "meeting" && <section id="meeting-content" className="content meeting-mode" tabIndex={-1}>
      <div className="meeting-top"><div><p className="overline">Short walkthrough · six steps · approximately seven minutes</p><h2>A short path through the revised proposition.</h2></div><p>The complete product remains available in the numbered rooms as the evidence and working layer beneath it.</p></div>
    ...
    {view === "proposal" && <section id="proposal-content" className="content proposal" tabIndex={-1}>
      ...
      <div className="ownership-grid"><article><p className="label">Purple Maiʻa would control</p><ul><li>Its engagement-specific records, decisions, configurations, language, and approved outputs</li><li>What remains internal, restricted, publishable, revisable, or retired</li><li>Any permitted use of Purple Maiʻa or community-defined material</li></ul></article><article><p className="label">The agreement would define</p><ul><li>Ownership or licensing of reusable software and implementation source</li><li>Confidentiality, retention, attribution, publication, and portfolio rights</li><li>Transfer, maintenance, deletion, migration, and closeout obligations</li></ul></article><article><p className="label">The tool would never do</p><ul><li>Decide who holds cultural or community authority</li><li>Convert consent into a one-time checkbox</li><li>Treat passage through a form as approval</li><li>Expose protected knowledge to prove accountability</li></ul></article></div>
    ...
    {view === "learning" && <section id="learning-content" className="content learning" tabIndex={-1}>
      ...
      <div className="sources"><p className="label">Public context informing this proposal</p><a href="https://luma.com/88dnl4w1" target="_blank" rel="noreferrer">ʻĀina Foundry · The Sovereign Stack ↗</a><a href="https://www.purplemaia.org/ainafoundry" target="_blank" rel="noreferrer">Purple Maiʻa · ʻĀina Foundry ↗</a><a href="https://blog.labs.purplemaia.org/" target="_blank" rel="noreferrer">ʻĀina Foundry · Prototype log ↗</a><a href="https://www.purplemaia.org/purple-blog/eahou-fest-2026-update-on-ai" target="_blank" rel="noreferrer">Purple Maiʻa · 2026 update on AI ↗</a><a href="https://www.purplemaia.org/purple-blog/data-guided-by-k%C4%81n%C4%81wai" target="_blank" rel="noreferrer">Purple Maiʻa · Data Guided by Kānāwai ↗</a><a href="https://www.civilbeat.org/2026/05/ai-data-centers-are-the-new-plantations-unless-we-build-them-differently/" target="_blank" rel="noreferrer">Civil Beat · Community governance of AI infrastructure ↗</a><a href="https://www.purplemaia.org/kula" target="_blank" rel="noreferrer">Purple Maiʻa · Kula ↗</a><a href="https://www.gida-global.org/careprinciples" target="_blank" rel="noreferrer">Global Indigenous Data Alliance · CARE Principles ↗</a><a href="https://localcontexts.org/labels/traditional-knowledge-labels/" target="_blank" rel="noreferrer">Local Contexts · TK Labels ↗</a></div>
    </section>}
    ...
    {view === "review" && <section id="review-content" className="content executive-review" tabIndex={-1}>
      <div className="section-intro compact"><p className="overline">Executive review · the whole proposition in one room</p><h2>Does the Sovereign Stack need an authority layer—or does existing work make this redundant?</h2><p>This is the shortest path through the prototype. The requested decision is narrow: correct, reject, refer, or pressure-test the premise. It is not a request to approve the framework, disclose protected knowledge, select KILO, or authorize a build.</p></div>
      <div className="executive-grid">
        <article><span>Public record</span><h3>Purple Maiʻa is building the technical Sovereign Stack.</h3><p>Its public work describes local infrastructure, open models, coding agents, edge systems, education, place-based practice, and sovereignty-first technology development.</p></article>
        ...
        <article><span>What remains unanswered</span><h3>Useful, redundant, or aimed at the wrong layer?</h3><p>Public material cannot settle that. A bounded, appropriately compensated discovery conversation could—if Purple Maiʻa identifies the question as worth testing and the right people hold the scope.</p></article>
      </div>
      ...
      <div className="decision-box"><div><p className="overline">The one-sentence proposition</p><h3>You built the stack. I independently built a prototype for carrying authority through it. Is there anything useful at the seam?</h3></div><p>The prototype demonstrates RN's legal-technical implementation thinking. Purple Maiʻa determines whether the seam is real, already addressed, or irrelevant.</p></div>
    </section>}
    ...
    <footer><div><span className="knot">◈</span><b>Authority Layer</b><small>Version 1.1 · 12 September 2026 · revised against newly public Sovereign Stack work</small></div><p>Independent prototype by Rayven-Nikkita (RN) Collins, developed from public materials. RN has previously been in contact with Purple Maiʻa. Purple Maiʻa has not commissioned, adopted, endorsed, or organizationally validated this prototype. Not a Purple Maiʻa product, policy, approved protocol, or community-authorized framework. <button onClick={openPrivacy} aria-controls="status-privacy-panel">Status, privacy &amp; erase drafts</button></p></footer>
  </main>;
}
